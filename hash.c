...
typedef struct thread_data {
    htab_t *h;       // Pointer to the hash table. This allows each thread to know which hash table it's working on.
    size_t start;    // The starting index (inclusive) of the hash table's buckets this thread will process. It indicates where the thread should begin its search.
    size_t end;      // The ending index (exclusive) of the hash table's buckets this thread will process. It indicates where the thread should stop its search.
    int search;      // The value to be searched for in the hash table.
} thread_data_t;

void* thread_search(void *arg) {
    thread_data_t *data = (thread_data_t*) arg; // input from arg
    htab_t *h = data->h; // data at hash table
    int search = data->search; // int searching for

    for (size_t i = data->start; i < data->end; ++i) { // i is data start value and loops to end value
        for (item_t *bucket = h->buckets[i]; bucket != NULL; bucket = bucket->next) { 
            if (bucket->value == search) { // if value of bucket equals the value seaching for
                printf("%s ", bucket->key); // print
            }
        }
    }
    return NULL;
...
int main(int argc, char **argv)
{
    ...
    size_t elementsPerThread = h.size / THREADS;

    for (int i = 0; i < THREADS; i++) {
        threadData[i].h = &h;
        threadData[i].start = i * elementsPerThread;
        threadData[i].end = (i == THREADS - 1) ? h.size : (i + 1) * elementsPerThread; // Let the last thread take the remaining elements
    ...
    printf("\n"); // Add a newline for clarity after all threads have printed their results
